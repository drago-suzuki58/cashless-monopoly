...
import type { BankLog, RegisterPayload, TransactionPayload, UndoPayload, Payload } from '../types';
...
interface BankState {
  ...
  history: BankLog[];
  processPayload: (payload: Payload) => { success: boolean, message: string, log?: BankLog };
  resetBank: () => void;
}
    ...
    (set, get) => ({
      ...
      processPayload: (payload: Payload) => {
        ...
        if (payload.act === 'tx') {
          ...
          if (!player) {
            return { success: false, message: 'プレイヤーが見つかりません。先に登録してください。' };
          }
        ...
        if (payload.act === 'undo') {
          ...
          if (!targetLog) {
            return { success: false, message: `取消対象の取引が見つかりません（seq:${tgt}）` };
          }

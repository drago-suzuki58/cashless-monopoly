    ...
    <div className="flex flex-col h-screen bg-gray-100 max-h-screen overflow-hidden">
      <header className="flex items-center justify-between p-4 bg-white shadow-sm shrink-0 z-10">
        <div className="flex items-center">
          <Link to="/" className="p-2 -ml-2 text-gray-500 hover:text-gray-900 rounded-full hover:bg-gray-100">
            <ArrowLeft size={24} />
          </Link>
      ...
      <main className="flex-1 flex flex-col p-4 space-y-6 overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto min-h-0 bg-white rounded-3xl shadow-sm border border-gray-200 p-4">
          <h2 className="text-sm font-bold text-gray-400 mb-4 px-2">プレイヤー残高</h2>
          {playersList.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-400 space-y-4">
              <p>プレイヤーが登録されていません</p>
              <p className="text-sm">プレイヤー端末で「登録用QR」を表示し、<br/>カメラにかざしてください</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-4">
              {playersList.map((p) => (
                ...
                >
                  ...
                  />
                  <div className="font-bold text-gray-700 text-lg mb-1 truncate">{p.name}</div>
      ...
      {toast && (
        <div key={toast.id} className="absolute inset-0 z-50 flex items-center justify-center p-6 pointer-events-none">
        ...
        <div className="absolute inset-0 z-40 flex flex-col bg-gray-50 animate-in slide-in-from-bottom-full duration-300">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {history.length === 0 ? (
              ...
              history.map((log) => (
                <div key={log.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col">
                  <div className="flex justify-between items-start mb-2">
                    <span className="font-bold text-gray-800">{log.playerName}</span>
